...
window.Packager = (function() {
  ...
  const assetManifest = {
    'turbowarp': {
      src: 'packager.e212db007a686ee5a5c1.js',
      size: 3040000 // TODO get accurate number, it's complicated because of gzip
    },
    'nwjs-win64': {
...
<!--
Generated by https://packager.turbowarp.org/
-->
...
<head>
  ...
  <style>
    ...
    body, [class^="gui_fullscreen-background"] {
...
<body>
  ...
  window.__PACKAGER__ = {
    ...
    id: ${stringify(this.options.projectId || '0')},
    username: ${stringify(this.options.username)},
    cloudHost: ${stringify(this.options.cloudHost)},
    ...
    handleProjectLoaded: function () {
      ...
      if (${stringify(this.options.infiniteClones)}) runtimeOptions.maxClones = Infinity;
      if (${stringify(this.options.removeFencing)}) runtimeOptions.fencing = false;
      if (${stringify(this.options.removeMiscLimits)}) runtimeOptions.miscLimits = false;
      vm.setRuntimeOptions(runtimeOptions);
  ...
  const pngToAppleICNS = async (pngData) => {
    ...
    for (const format of eligibleFormats) {
      ...
      const icnsImage = await Icns.IcnsImage.fromArrayBuffer(arrayBuffer, format.type);
